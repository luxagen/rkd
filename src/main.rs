...
impl RKD
{
	...
	fn diff(&mut self,logL: &Vec<&str>,logR: &Vec<&str>) -> i32
	{
		...
		{
			eprintln!("[WARNING] Ambiguous files: < {}, > {}",fakeHashCountL,fakeHashCountR);
		}
...
impl LogLine
{
	fn parse<'a>(input: &'a str,fakeHashCount: &mut usize,side: usize) -> nom::IResult<&'a str,Option<Self>>
	{
		use nom::{
			sequence::*,
			character::complete::*,
